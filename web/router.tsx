...
import ReportPage from "./tools/report/index.tsx";

const TOOL_PAGES: Record<string, React.ComponentType> = {
	RESEARCH_RUN: ReportPage,
};

function ToolRouter() {
	const { toolName, status } = useMcpState();

	// Any tool result renders the report view (RESEARCH_RUN is the primary one).
	const Page = (toolName && TOOL_PAGES[toolName]) || ReportPage;

	if (!toolName && status === "initializing") {
		return (
